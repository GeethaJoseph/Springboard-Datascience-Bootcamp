...
SELECT name AS 'Faclities that charge a fee to members'
FROM Facilities
WHERE membercost !=0
...
SELECT COUNT( name ) AS 'No of faclities free to members'
FROM Facilities
WHERE membercost =0
...
JOIN Members ON Bookings.memid = Members.memid
AND Bookings.facid = Facilities.facid
HAVING Bookings.starttime LIKE '%2012-09-14%'
AND Cost >30
ORDER BY Cost DESC
...
CASE WHEN SUM(sub.Cost) - sub.Facmaintenance < 1000
THEN SUM(sub.Cost) - sub.Facmaintenance 
ELSE '>1000'
END AS Total_Revenue
  FROM (

SELECT Facilities.name as Facility_name,
      Facilities.monthlymaintenance as Facmaintenance,
      CASE WHEN Members.memid =0      
THEN Facilities.guestcost * Bookings.slots
ELSE Facilities.membercost * Bookings.slots
END AS Cost
